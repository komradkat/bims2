...
class BusinessPermit(models.Model):
    ...
    dti_sec_number = models.CharField(
        max_length=50, blank=True, verbose_name="DTI/SEC Registration Number"
    )
    ...
    clearance_fee = models.DecimalField(
    ...
    def save(self, *args, **kwargs):
        if not self.permit_number:
            ...
            last_permit = (
                BusinessPermit.objects.filter(permit_number__startswith=f"BP-{year}")
...
class BusinessClearance(models.Model):
    ...
    permit = models.ForeignKey(
        BusinessPermit, on_delete=models.CASCADE, related_name="clearances"
    )
