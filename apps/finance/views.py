...
class FinanceDashboardView(LoginRequiredMixin, NonBootstrapRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Statistics data
        daily_collection = (
            OfficialReceipt.objects.filter(date__date=today, status="paid").aggregate(
                Sum("amount")
            )["amount__sum"]
            or 0
        )
        ...
        monthly_revenue = (
            OfficialReceipt.objects.filter(
...
class RevenueAnalyticsView(LoginRequiredMixin, NonBootstrapRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        trends = (
            OfficialReceipt.objects.filter(date__gte=six_months_ago, status="paid")
        ...
        source_breakdown = (
            OfficialReceipt.objects.filter(status="paid")
            .values("particulars")
        ...
        total_all_time = (
            OfficialReceipt.objects.filter(status="paid").aggregate(Sum("amount"))[
                "amount__sum"
            ]
            or 0
        )
