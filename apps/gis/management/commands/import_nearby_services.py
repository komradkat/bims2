...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--radius', type=int, default=5000, help='Radius in meters for the search (default: 5000)')
    ...
    def handle(self, *args, **options):
        ...
        if not info or not info.latitude or not info.longitude:
            self.stdout.write(self.style.ERROR('Error: Barangay coordinates are not set. Please configure them in Setup.'))
            return

        lat = info.latitude
        lng = info.longitude

        self.stdout.write(f"Searching for emergency services within {radius}m of {lat}, {lng}...")
        ...
        for el in elements:
            ...
            name = tags.get('name') or tags.get('operator') or f"Unknown {tags.get('amenity')}"
            osm_type = tags.get('amenity')
            service_type = type_map.get(osm_type, 'police') # Fallback to police if unknown
            ...
            if dry_run:
                self.stdout.write(f"[Dry Run] Found: {name} ({service_type}) at {plat}, {plng}")
                continue
