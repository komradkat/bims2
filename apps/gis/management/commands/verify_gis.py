...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        residents_with_coords = Resident.objects.filter(
        ...
        if residents_with_coords == 0:
            self.stdout.write(
                self.style.WARNING(
                    "WARNING: No residents have coordinates. Map will be empty."
                )
            )
            ...
            try:
                ...
                resident.save()
                self.stdout.write(
                    self.style.SUCCESS(f"Added coordinates to resident: {resident}")
                )
        ...
        if response.status_code == 200:
            ...
            if len(content.get("features")) > 0:
                self.stdout.write(
                    self.style.SUCCESS("SUCCESS: Data is being returned correctly.")
                )
            else:
                self.stdout.write(
                    self.style.WARNING("WARNING: GeoJSON is empty (but valid).")
                )
        else:
            self.stdout.write(
                self.style.ERROR(f"FAILED: API returned {response.status_code}")
            )
