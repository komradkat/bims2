...
class BlotterListView(LoginRequiredMixin, NonBootstrapRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = BlotterCase.objects.all().prefetch_related(
            "complainants", "respondents", "hearings"
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["stats"] = {
            ...
            "urgent_hearings": Hearing.objects.filter(
                status="scheduled", scheduled_at__date=timezone.now().date()
            ).count(),
        }
...
class BlotterCreateView(LoginRequiredMixin, NonBootstrapRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            context["complainant_form"] = ComplainantForm(
                self.request.POST, prefix="complainant"
            )
            context["respondent_form"] = RespondentForm(
                self.request.POST, prefix="respondent"
            )
    ...
    def form_valid(self, form):
        ...
        if complainant_form.is_valid() and respondent_form.is_valid():
            ...
            if hearing_form.is_valid() and hearing_form.cleaned_data.get(
                "scheduled_at"
            ):
            ...
            messages.success(
                self.request,
                f"New blotter case {self.object.case_number} has been recorded.",
            )
...
class CaseStatusUpdateView(LoginRequiredMixin, NonBootstrapRequiredMixin, View):
    def post(self, request, pk):
        ...
        if new_status in dict(BlotterCase.STATUS_CHOICES):
            ...
            messages.success(
                request,
                f"Case {case.case_number} status updated to {case.get_status_display()}.",
            )
