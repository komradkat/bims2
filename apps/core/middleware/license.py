...
class LicenseVerificationMiddleware:
    ...
    def __call__(self, request):
        ...
        if settings.DEBUG or license_debug_bypass:
            debug_tier = getattr(settings, "BIMS_DEBUG_TIER", "ultra")
            request.license = {
                "tier": debug_tier,
                "max_users": 999
                if debug_tier == "ultra"
                else (25 if debug_tier == "pro" else 5),
                "expiry_date": None,
                "key_preview": f"DEBUG MODE ({debug_tier.capitalize()})",
            }
            ...
            try:
                ...
                if license_key.is_valid():
                    license_data = {
                        ...
                        "expiry_date": str(license_key.expiry_date)
                        if license_key.expiry_date
                        else None,
                        "key_preview": license_key.key[:8] + "..."
                        if len(license_key.key) > 8
                        else license_key.key,
                    }
                    # Cache for 1 hour
