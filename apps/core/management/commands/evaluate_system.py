...
class Command(BaseCommand):
    ...
    def evaluate_prerequisites(self):
        ...
        user, created = User.objects.get_or_create(
            username="eval_admin",
            defaults={"email": "admin@eval.local", "role": "admin"},
        )
        if created:
    ...
    def evaluate_blotter(self):
        ...
        Respondent.objects.create(
            case=case, name="John Doe (Non-resident)", contact_number="09123456789"
        )
        ...
        if case.next_hearing == hearing:
            self.stdout.write(
                self.style.SUCCESS(" - Logic Check Passed: next_hearing property works")
            )
        else:
            self.stdout.write(
                self.style.ERROR(
                    " - Logic Check Failed: next_hearing property mismatch"
                )
            )
    ...
    def evaluate_finance(self):
        ...
        fee, _ = Fee.objects.get_or_create(
            name="Test Fee", defaults={"default_amount": 50.00}
        )
    ...
    def evaluate_certificates(self):
        ...
        if cert.transaction_number.startswith("CERT-"):
            self.stdout.write(
                self.style.SUCCESS(" - Logic Check Passed: Transaction ID generation")
            )
        else:
            self.stdout.write(
                self.style.ERROR(" - Logic Check Failed: Transaction ID generation")
            )
    ...
    def evaluate_business(self):
        ...
        if not permit.is_expired:
            self.stdout.write(
                self.style.SUCCESS(" - Logic Check Passed: Expiry logic correct")
            )
        else:
            self.stdout.write(
                self.style.ERROR(
                    " - Logic Check Failed: Permit marked expired prematurely"
                )
            )
