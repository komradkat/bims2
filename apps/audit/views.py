...
class AuditLogsView(LoginRequiredMixin, NonBootstrapRequiredMixin, View):
    ...
    def _build_logs(
            ...
            try:
                ...
                for record in records:
                    ...
                    if (
                        search
                        and search.lower()
                        not in (user_display + details_str + module_name).lower()
                    ):
                        continue
                    all_logs.append(
                        {
        ...
        if not module_filter or module_filter in ("All", "System", "Database"):
            ...
            if search:
                qs = qs.filter(
                    Q(user__username__icontains=search) | Q(details__icontains=search)
                )
            if user_filter:
                ...
                all_logs.append(
                    {
                        ...
                        "module": "Database"
                        if event.action.startswith("DB_")
                        else "System",
                        "details": details_str,
    ...
    def _export_response(self, logs, fmt="csv"):
        ...
        if fmt == "json":
            ...
            writer.writerow(
                ["Timestamp", "User", "Module", "Action", "Details", "IP Address"]
            )
            for log in logs:
                writer.writerow(
                    [
            ...
            log_rows.append(
                {
                    ...
                    "action_icon": ACTION_ICON.get(label, "·"),
                    "details": ev.details.get("message", label)
                    if ev.details
                    else label,
        ...
        if "sqlite" in engine:
            ...
            try:
                ...
                log_system_event(
                    ...
                    request=request,
                    details={
                        "message": f"SQLite backup downloaded: {filename}",
                        "engine": "sqlite3",
                        "filename": filename,
                    },
                )
            ...
            log_system_event(
                ...
                request=request,
                details={
                    "message": f"Backup initiated for {engine} (dumpdata)",
                    "engine": engine,
                },
            )
    ...
    def post(self, request):
        ...
        try:
            ...
            log_system_event(
                ...
                request=request,
                details={
                    "message": f"Dumpdata export ({fmt})",
                    "apps": selected_apps or "all",
                    "format": fmt,
                },
            )
...
class DatabaseImportView(LoginRequiredMixin, NonBootstrapRequiredMixin, View):
    def post(self, request):
        ...
        if ext not in allowed_exts:
            messages.error(
                request,
                f"Unsupported file type '{ext}'. Accepted: .json, .yaml, .xml, .sqlite3",
            )
            return redirect("audit:database")
        ...
        try:
            ...
            if ext == ".sqlite3":
                ...
                messages.success(
                    request,
                    f"✓ '{fixture.name}' restored as the active SQLite database.",
                )
                ...
                messages.success(
                    request,
                    f"✓ '{fixture.name}' ({fmt.upper()}) imported successfully.",
                )
