...
class Resident(models.Model):
    ...
    educational_attainment = models.CharField(
        max_length=50, choices=EDUCATIONAL_ATTAINMENT_CHOICES, blank=True
    )
    ...
    # Government IDs
    philhealth_no = models.CharField(
        max_length=20, blank=True, verbose_name="PhilHealth Number"
    )
    sss_gsis_no = models.CharField(
        max_length=20, blank=True, verbose_name="SSS/GSIS Number"
    )
    ...
    def clean(self):
        ...
        if self.photo and self.photo.size > MAX_UPLOAD_SIZE:
            raise ValidationError(
                {
                    "photo": f"Image file too large. Max size is {MAX_UPLOAD_SIZE / 1024 / 1024}MB."
                }
            )
...
class HouseholdMember(models.Model):
    ...
    household_head = models.ForeignKey(
        Resident, on_delete=models.CASCADE, related_name="household_membership_as_head"
    )
