...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        middle_names = ['Santos', 'Cruz', 'Reyes', 'Garcia', 'Lopez', 'Ramos', 'Torres', 'Flores']
        last_names = ['Dela Cruz', 'Santos', 'Reyes', 'Garcia', 'Lopez', 'Martinez', 'Hernandez', 'Gonzales']
        ...
        for i in range(count):
            ...
            resident = Resident.objects.create(
                ...
                occupation=random.choice(['Farmer', 'Driver', 'Teacher', 'Vendor', 'Carpenter']),
                educational_attainment=random.choice(['high_school_graduate', 'college_level', 'elementary_graduate']),
        ...
        for head in household_heads:
            ...
            for j in range(num_members):
                ...
                member = Resident.objects.create(
                    ...
                    date_of_birth=dob,
                    sex=random.choice(['M', 'F']),
                    civil_status='single' if age < 18 else random.choice(['single', 'married']),
                    citizenship='Filipino',
                    ...
                    household_head=head,
                    relationship_to_head=random.choice(['Spouse', 'Son', 'Daughter', 'Parent']),
                    mobile_number=f'0917{random.randint(1000000, 9999999)}' if age >= 18 else '',
                    is_pwd=random.choice([True, False]) if random.random() < 0.1 else False,
                    disability_type='Visual Impairment' if random.random() < 0.5 else 'Orthopedic',
                    is_4ps=random.choice([True, False]) if random.random() < 0.2 else False,
                    occupation=random.choice(['Student', 'None', 'Helper']) if age < 18 else random.choice(['Farmer', 'Driver', 'Vendor']),
                    educational_attainment='elementary_level' if age < 12 else random.choice(['high_school_level', 'college_level']),
                    employment_status='student' if age < 18 else random.choice(['employed', 'unemployed'])
                )
                ...
                self.stdout.write(f'  Created member: {member.full_name} ({member.relationship_to_head})')
        
        total = Resident.objects.count()
        self.stdout.write(self.style.SUCCESS(f'\nTotal residents created: {total}'))
        self.stdout.write(self.style.SUCCESS(f'Household heads: {len(household_heads)}'))
        self.stdout.write(self.style.SUCCESS(f'Household members: {total - len(household_heads)}'))
