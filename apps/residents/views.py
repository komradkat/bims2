...
from .forms import ResidentForm, HouseholdHeadForm, HouseholdMemberFormSet
...
class ResidentsListView(LoginRequiredMixin, NonBootstrapRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = Resident.objects.filter(is_active=True).select_related(
            "household_head"
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["puroks"] = Purok.objects.values_list("name", flat=True).order_by(
            "name"
        )
        ...
        if total_residents > 0:
            ...
            # Sector Stats
            context["senior_count"] = all_residents.filter(
                is_senior_citizen=True
            ).count()
            context["pwd_count"] = all_residents.filter(is_pwd=True).count()
            context["solo_parent_count"] = all_residents.filter(
                is_solo_parent=True
            ).count()
            ...
            context["age_group_counts"] = {
                ...
                "Youth": all_residents.filter(
                    date_of_birth__lte=child_limit, date_of_birth__gt=youth_limit
                ).count(),
                "Adults": all_residents.filter(
                    date_of_birth__lte=youth_limit, date_of_birth__gt=adult_limit
                ).count(),
            ...
            # Purok Distribution
            purok_dist = (
                all_residents.values("purok_link__name")
...
def export_residents_excel(request):
    ...
    for row_num, resident in enumerate(residents, 2):
        ...
        ws.cell(
            row=row_num, column=6, value=resident.date_of_birth.strftime("%Y-%m-%d")
        )
        ...
        ws.cell(
            row=row_num, column=14, value="Yes" if resident.is_senior_citizen else "No"
        )
        ws.cell(row=row_num, column=15, value="Yes" if resident.is_pwd else "No")
        ws.cell(
            row=row_num, column=16, value="Yes" if resident.is_solo_parent else "No"
        )
        ...
        ws.cell(
            row=row_num,
            column=20,
            value=resident.get_educational_attainment_display()
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.object.is_household_head:
            context["household_members"] = Resident.objects.filter(
                household_head=self.object, is_active=True
...
class ResidentSearchView(LoginRequiredMixin, NonBootstrapRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return Resident.objects.filter(filters, is_active=True).order_by(
            "last_name", "first_name"
        )[:10]


class HouseholdRegistrationView(
    LoginRequiredMixin, NonBootstrapRequiredMixin, CreateView
):
    """
