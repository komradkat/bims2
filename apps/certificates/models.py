...
certificate_storage = FileSystemStorage(
    location=str(
        getattr(
            settings,
            "BIMS_CERTIFICATE_STORAGE_ROOT",
            settings.MEDIA_ROOT / "certificates/issued",
        )
    ),
...
class Certificate(models.Model):
    ...
    resident = models.ForeignKey(
        Resident, on_delete=models.CASCADE, related_name="certificates"
    )
    ...
    or_number = models.CharField(
        max_length=50, blank=True, help_text="Official Receipt Number"
    )
    ...
    digital_hash = models.CharField(
        max_length=64, blank=True, help_text="SHA256 hash of the generated document"
    )
    document = models.FileField(
        storage=certificate_storage, upload_to="%Y/%m/", null=True, blank=True
    )
