...
class CertificatePrintView(LoginRequiredMixin, NonBootstrapRequiredMixin, DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if captain_official:
            ...
            captain_title = captain_official.get_position_display().split(" / ")[
                -1
            ]  # Prefer Barangay Captain if available
        elif info:
        ...
        try:
            ...
            if certificate.document and certificate.digital_hash:
                try:
                    return HttpResponse(
                        certificate.document.open("rb"), content_type="application/pdf"
                    )
                except Exception:
            ...
            response["Content-Disposition"] = (
                f'inline; filename="{name_slug}-{type_slug}-{txn}.pdf"'
                if "name_slug" in locals()
...
class CertificateCenterView(LoginRequiredMixin, NonBootstrapRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return CertificateType.objects.filter(is_active=True).order_by(
            "default_price", "name"
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Successfully issued {certificate.certificate_type.name} for {certificate.resident.full_name}.",
            )
            from django.urls import reverse

            return redirect(
                reverse("certificates:print", kwargs={"pk": certificate.pk})
            )
        else:
...
class CertificateListView(LoginRequiredMixin, NonBootstrapRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = Certificate.objects.all().select_related(
            "resident", "certificate_type", "issued_by"
        )
...
def void_certificate(request, pk):
    if request.method == "POST":
        ...
        certificate.save()
        messages.warning(
            request, f"Certificate {certificate.transaction_number} has been voided."
        )
